...
upload_basecalled_data2IRIDA () {
   	...
   	touch ${1}/CompletedJobInfo.xml #create dummy file in the NextSeq run directory 
	sleep 10
    runid=$(sbatch -c 1 --mem=16G -p "Reference/Surveillance" -J "NextSeqDataUpload" --time=24:00:00 -o "${logfilename}" --wrap="source activate irida-uploader-0.8.3  && irida-uploader --config ./irida-uploader-config.conf --readonly -f -d ${1}"| grep -oE  "[0-9]+")
...
if [ -d "$runfolder" ]; then
  	echo "Folder ${runfolder} exists"
        ...
        if [ ! -f  "${runfolder}/SampleSheet.csv"  ];then echo "SampleSheet.csv file does not exist or named differently"; exit 1; fi;    
	...
	runid=$(sbatch -c 16 --mem=64G  --time=24:00:00 -p "Reference/Surveillance" -J "NextSeqBasecalling" -o "${basecall_log_file}" --wrap="source activate illumina && cd ${runfolder} && bcl2fastq --no-lane-splitting --sample-sheet SampleSheet.csv" | grep -oE  "[0-9]+")
        #TODO: switch the queue if run did not start in 5 minutes. Explore other queues include NMLResearch and ExternalR (do not touch Outbreak queue)
	...
	while [ $bcstatus != 0 ];do
	  ...
	  squeue | grep gnextseq #check queue status (if job is running (R) or in queue (PD))
          sleep 60
	done
	...
	if [ ${#fastqs[@]} -eq 0 ] || [ ${#destination_dir} -eq 0 ];then
		...
		echo "Running automatic sample upload to IRIDA server in 60s. "
		sleep 60
